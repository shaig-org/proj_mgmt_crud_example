...
**Cross-stack all-at-once**: `./devtools/run-all-validations.sh` runs backend + frontend + dashboard fast checks in parallel (~38s, backend dominates). Pass `--with-e2e` to additionally include Playwright (~75s for E2E itself, ~115s total: fast checks complete first, then E2E runs). Useful when you've touched multiple modules and want one command for the whole repo.
