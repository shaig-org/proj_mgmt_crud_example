...
| `index.html` | ❌ gitignored | Regenerated static viewer. Self-contained (inline CSS), just open in a browser. |
...
```bash
# Check current state vs baseline. Writes report.json + index.html.
...
Open `index.html` in any browser. Columns:
...
3. If it exits 1 or shows a diff you expected, open `index.html` to confirm the diff matches intent.
...
2. Open the updated `index.html` (or `git diff baseline.json`) to see the
