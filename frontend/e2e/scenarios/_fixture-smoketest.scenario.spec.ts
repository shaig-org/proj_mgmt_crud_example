...
import { scenarioTest, EVIDENCE_DIR, type ScenarioMetadata } from '../helpers/scenario';
...
test.describe('scenarioTest fixture contract', () => {
  ...
  test('scenario_fixture_produces_numbered_screenshots_per_step', async () => {
    ...
    const dir = path.join(EVIDENCE_DIR, 'screenshots', shared.slug);
  ...
  test('scenario_fixture_writes_metadata_json_with_expected_fields', async () => {
    const metadataPath = path.join(EVIDENCE_DIR, 'metadata', `${shared.slug}.json`);
  ...
  test('scenario_fixture_copies_video_and_trace_to_stable_paths', async () => {
    const videoPath = path.join(EVIDENCE_DIR, 'videos', `${shared.slug}.webm`);
    const tracePath = path.join(EVIDENCE_DIR, 'traces', `${shared.slug}.zip`);
