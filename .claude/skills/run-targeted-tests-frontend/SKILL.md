...
```bash
cd frontend && npx playwright test e2e/projects.spec.ts --reporter=list
```

**One test by name** → grep on title:
```bash
cd frontend && npx playwright test --grep "creates project via form" --reporter=list
```

**One browser** (faster than all three):
```bash
cd frontend && npx playwright test e2e/projects.spec.ts --project=chromium --reporter=list
```

**Quick iteration on a test you just wrote** → same file, chromium only, one worker:
```bash
cd frontend && npx playwright test e2e/projects.spec.ts --project=chromium --workers=1 --reporter=list
```
...
## Rules
- Always `--reporter=list`. Never let the HTML reporter serve — it blocks.
...
```bash
cd frontend && npx playwright test e2e/projects.spec.ts --project=chromium --workers=1 --reporter=list --trace=on
```
